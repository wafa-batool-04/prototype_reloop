<?php
// config/db.php - with automatic product installation
class Database {
    private $host = "127.0.0.1";
    private $db_name = "prototype_reloop";
    ...
    public function getConnection() {
        ...
        try {
            ...
            // AUTO-INSTALL PRODUCTS if they are missing
            $this->autoInstallProducts();
            
        } catch(PDOException $exception) {
            // Database doesn't exist or connection failed
            die("<div style='text-align:center; padding:50px; font-family:Arial;'>
                    <h1>🔧 Database Not Found</h1>
                    <p>Please follow these steps to set up the project:</p>
                    <ol style='text-align:left; display:inline-block;'>
                        <li>Open phpMyAdmin</li>
                        <li>Create a database named: <strong>prototype_reloop</strong></li>
    ...
    // Auto-install products if they don't exist
    private function autoInstallProducts() {
        try {
            ...
            // Check if product ID 100 exists
            $check = $this->conn->prepare("SELECT COUNT(*) as count FROM products WHERE id = 100");
            $check->execute();
            $result = $check->fetch();
            
            if ($result['count'] == 0) {
                // Products are missing - install them!
                $this->installProducts();
            }
        } catch (PDOException $e) {
            // Silent fail - don't break the site if auto-install fails
            error_log("Auto-install check failed: " . $e->getMessage());
        }
    }
    ...
    private function installProducts() {
        ...
        foreach ($products as $product) {
            try {
                $stmt = $this->conn->prepare("INSERT INTO products (id, user_id, name, brand, category, price, original_price, discount, description, warranty, stock_status, image_url, image_url_2, image_url_3, created_at) 
                                              VALUES (?, 1, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())");
                $stmt->execute([$product[0], $product[1], $product[2], $product[3], $product[4], $product[5], $product[6], $product[7], $product[8], $product[9], $product[10], $product[11], $product[12]]);
                $inserted++;
            } catch (PDOException $e) {
        ...
        $this->conn->exec("ALTER TABLE products AUTO_INCREMENT = 200");
        
        if ($inserted > 0) {
            error_log("Auto-installed $inserted products to database");
        }
    }
}
?>